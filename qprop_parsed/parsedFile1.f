...
REAL TP_C(N), TP_B(N),QP_C(N), QP_B(N)
# part of multi line command.
LOGICAL STALL(N)
C
C-----------------------------------------------------
...
DO I = 1, N
BTOT = B(I) + DBE
CALL GVCALC(C(I),BTOT,R(I),BLDS,RAD,VEL,OMG,VSO,CL0(I),DCLDA(I),CLMIN(I),CLMAX(I),MCRIT(I),GAM  ,GAM_VEL,GAM_OMG,GAM_C,GAM_B,VA(I), VA_VEL, VA_OMG, VA_C, VA_B,VT(I), VT_VEL, VT_OMG, VT_C, VT_B,CL(I), CL_VEL, CL_OMG, CL_C, CL_B, STALL(I), LCONV)
# part of multi line command.
# part of multi line command.
# part of multi line command.
# part of multi line command.
...
CALL DCDFUN(BTOT,WA,WT, CLCD0(I),CL0(I),DCLDA(I),DCD,DCD_B,DCD_WA,DCD_WT)
# part of multi line command.
C
CD(I)  = CD(I)  + DCD
...
FA_VEL = HRC*W_VEL*(  CL(I) *WT     - CD(I) *WA    )+ HRC*W    *(  CL_VEL*WT     - CD_VEL*WA+ CL(I) *WT_VEL - CD(I) *WA_VEL)
# part of multi line command.
# part of multi line command.
FA_OMG = HRC*W_OMG*(  CL(I) *WT     - CD(I) *WA    )+ HRC*W    *(  CL_OMG*WT     - CD_OMG*WA+ CL(I) *WT_OMG - CD(I) *WA_OMG)
# part of multi line command.
# part of multi line command.
FA_B   = HRC*W_B  *(  CL(I) *WT     - CD(I) *WA    )+ HRC*W    *(  CL_B  *WT     - CD_B  *WA+ CL(I) *WT_B   - CD(I) *WA_B  )
# part of multi line command.
# part of multi line command.
FA_C   = HRC*W_C  *(  CL(I) *WT     - CD(I) *WA    )+ HRC*W    *(  CL_C  *WT     - CD_C  *WA+ CL(I) *WT_C   - CD(I) *WA_C  )+ 0.5*RHO*W    *(  CL(I) *WT     - CD(I) *WA    )
# part of multi line command.
# part of multi line command.
# part of multi line command.
C
FT     = HRC*W    *(  CL(I) *WA     + CD(I) *WT    )
FT_VEL = HRC*W_VEL*(  CL(I) *WA     + CD(I) *WT    )+ HRC*W    *(  CL_VEL*WA     + CD_VEL*WT+ CL(I) *WA_VEL + CD(I) *WT_VEL)
# part of multi line command.
# part of multi line command.
FT_OMG = HRC*W_OMG*(  CL(I) *WA     + CD(I) *WT    )+ HRC*W    *(  CL_OMG*WA     + CD_OMG*WT+ CL(I) *WA_OMG + CD(I) *WT_OMG)
# part of multi line command.
# part of multi line command.
FT_B   = HRC*W_B  *(  CL(I) *WA     + CD(I) *WT    )+ HRC*W    *(  CL_B  *WA     + CD_B  *WT+ CL(I) *WA_B   + CD(I) *WT_B  )
# part of multi line command.
# part of multi line command.
FT_C   = HRC*W_C  *(  CL(I) *WA     + CD(I) *WT    )+ HRC*W    *(  CL_C  *WA     + CD_C  *WT+ CL(I) *WA_C   + CD(I) *WT_C  )+ 0.5*RHO*W    *(  CL(I) *WA     + CD(I) *WT    )
# part of multi line command.
# part of multi line command.
# part of multi line command.
C
