...
PROGRAM QPROP
	...
	IF(ERROR) THEN
		...
		DO I = 1, N
			IRE = INT( REREF(I) )
			print ' {:3d}{:9.4f}{:9.4f}{:9.3f}{:9d}'.format( I, R(I), C(I), B(I)*180.0/PI, IRE)
	...
	DO IMPAR=1, NMPAR
		LU.write('#{:12.5f} {}'.format( PARMOT(IMPAR), PMLAB(IMPAR)))
	ENDDO
	LU.write('#{}{}{}{}'.format())
	...
	IF(LRDUMP) THEN
		...
		DO I = 1, N
			...
			C
			LU.write(' {:8.4f}{:8.4f}{:8.3f}{:9.4f}{:9.5f}{:9d}{:7.3f}{:9.4f}{:9.4f}{:12.4f}{:12.4f}{:12.4f}'.format(RU,   CU,   BU, CL(I), CD(I),IRE,  AMA, EFFI, EFFP, WA, ASWIRL, ADW))
			# part of multi line command.
			# part of multi line command.
	...
	C
	900  CONTINUE
	print '\nReaderror\ninfile:{}\nline{:3d}:{}'.format( FILNAM(1:64), ILINE, LINE)
	...
	C
	950  CONTINUE
	print '\nUnexpectedend-of-filereached\ninfile:{}\nline{:3d}'.format( FILNAM(1:64), ILINE)
